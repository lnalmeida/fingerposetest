...
async function main() {
    ...
    const estimateHands = async () => {
        ...
        for (const hand of hands) {
            ...
            const keypoints3D = hands.keypoints3D.map(keypoint => [keypoint.x, keypoint.y, keypoint.z])
...
function updateDebugInfo(data, hand) {
    ...
    for (let fingerIdx in data) {
        document.querySelector(
            `${summaryTable} span#curl-${fingerIdx}`
        ).innerHTML = data[fingerIdx][1];
        document.querySelector(
            `${summaryTable} span#dir-${fingerIdx}`
        ).innerHTML = data[fingerIdx][2];
    }
}
